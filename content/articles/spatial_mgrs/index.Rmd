...
countries <- ne_countries(scale = "medium", continent = "Europe", returnclass = "sf")
...
Confusion between UTM and MGRS is further compounded by the fact that some (civil) sources that distinguish these grids, erroneously describe the division of UTM zones with 8° wide latitude bands as being part of the UTM map projection system, while this belongs solely to the MGRS.
...
```{r grid2, fig.cap="The same map of Europe in the projected CRS 'EPSG:3035'. The grid however is from 'EPSG:3034' (ETRS89-extended / LCC Europe)."}
ggplot(data = countries) +
  geom_sf(fill = "grey97", colour = "grey50") +
  coord_sf(datum = "EPSG:3034", xlim = c(2.5e6, 6e6), ylim = c(1.5e6, 4.5e6))
```

```{r graticule, fig.cap="'Grid lines' of geographic CRSs are called the graticule. This map still uses the projected CRS 'EPSG:3035' but displays the graticule of geographic CRS 'EPSG:4258' (ETRS89)."}
ggplot(data = countries) +
  geom_sf(fill = "grey97", colour = "grey50") + 
...
As an example, we calculate the projected X and Y coordinates of a point in Belgium using the `sf` package in R, at the millimeter resolution.
...
```{r flanders, fig.cap="A map in the Belgian CRS 'EPSG:3812' (ETRS89 / Belgian Lambert 2008), showing the provinces of Flanders. The point with geographical coordinates '51.1°N 4.1°E' is shown in red. It is situated in the province 'Oost-Vlaanderen'. The superposed UTM grid lines and UTM coordinates are from CRS 'EPSG:25831' (ETRS89 / UTM zone 31N), with lines every 5 km."}
ggplot(provinces) +
  geom_sf() +
  geom_sf(
...
```{r utm-grid-europe, fig.cap="UTM grid in the European area, using cells of 100 km. As UTM zones each have their own CRS (in one hemisphere), an angle arises between grid lines in two different UTM zones."}
ggplot(data = countries) +
  geom_sf(fill = "grey97", colour = "grey50") + 
...
Hence for the Belgian point referred above with UTM coordinates <code>`r st_transform(point, "EPSG:25831") |> st_coordinates() |> format(nsmall = 3, trim = TRUE) |> paste(collapse = ", ")`</code>, the UTM grid reference at 10 meter precision is <code>`r paste("31", floor(st_transform(point, "EPSG:25831") |> st_coordinates() / 10) |> paste(collapse = " "))`</code>.
...
[^hemisphere]: Some sources [e.g. @uf_geoplan_center_mgrs_2009] therefore mention the possibility to extend the UTM zone with 'N' or 'S' depending on the hemisphere, resulting in e.g. <code>`r paste("31N", floor(st_transform(point, "EPSG:25831") |> st_coordinates() / 10) |> paste(collapse = " "))`</code>. However this approach is now officially obsoleted, and _full_ (i.e. globally unique) UTM coordinates should contain an MGRS grid zone prefix (see MGRS section) instead [@national_geospatial-intelligence_agency_universal_2014-1].
