...
```{r graticule, fig.cap="'Grid lines' of geographic CRSs are called the graticule, which consists of parallels and meridians. This map still uses the projected CRS 'EPSG:3035' but displays the graticule of geographic CRS 'EPSG:4258' ('ETRS89'), projected in CRS 'EPSG:3035'."}
ggplot(data = countries) +
  geom_sf(fill = "grey97", colour = "grey50") + 
...
As an example, we calculate the projected easting (X) and northing (Y) of a point in Belgium using the `sf` package in R, at the millimeter resolution.
