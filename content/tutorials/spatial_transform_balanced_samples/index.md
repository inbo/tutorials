...
Below, I will demonstrate how to create a circular area covered with random points, by means of a simple polar coordinate transformation.

### (2) Find Your Spatial Balance

A second feature of the initial example random points is that they cluster randomly.
This means that some of the points are close together, and other areas are undersampled.

This has disadvantages, e.g. if you want to sample information about a geographic area.
Points proximal to existing measurements provide very little extra information (at least if [Tobler's Law](https://tutorials.inbo.be/tutorials/spatial_variograms) holds).
...
What we often desire is **spatially balanced sampling**:
...
<figcaption>
Figure 1: Spatially balanced sampling in a circular area.
</figcaption>
...
The figure above treats all points in the circle equally.
But then, there are special points in a circular area - obviously, its center.
...
Throughout this tutorial, I will also apply some basic concepts of functional programming.
(Really just a tiny, superficial addition, nothing to worry about.)
...
This method is a rather recent addition to the toolbox ([Robertson *et al.*, 2013](#ref-robertson2013)).
...
This also means that you can add samples to the end of the series later on.
...
the function `apply_transform` takes a function as an input argument.
It then applies it, but also does some more stuff (here: scaling).
We just invented a simple "wrapper".

Finally, we get our `x` and `y` back:
...
Note how the sample is still spatially balanced, but in an unexpected way:
...
It is high, close to the center, and reduces towards the edges of our circle.
...
To support my own sparse maths memory, I like to trial-and-error my way out.
...
So to get the distribution uniform, we apply the inverse.
(The skalar \\(\pi\\) does not matter because of the distance normalization above.)
...
title("the red and grey points are the same, before and after transformation")
```
...
Unfortunately, I did not manage to get periodical, concentric bands of points yet, but I suspect they require complex numbers.

``` r
trial( function(x) x + sin(16 * x) ) 
```

<img src="index.markdown_strict_files/figure-markdown_strict/periodicity-1.png" width="768" />

You now possess the basic tooling and can take these ideas and concepts further, for your own good.

<a id="sec-summary"></a>

# Summary

In this tutorial, I assembled several simple but useful tools to homogeneously, but randomly distribute a given number of points in a circular area.
...
After all these choices and adjustments, can we still talk about a random distribution?
