---
title: "How to use open vector file formats in R: GeoTIFF & GeoPackage"
...
GDAL's GKG-raster driver itself is still less worked out than its drivers for GeoTIFF or GPKG-vector (note that one GPKG file can accommodate both layer types).
...
### Writing one raster layer rom a `RasterLayer` object
...
But, we're just so lucky that Edzer Pebesma -- the creator of `sf` -- has also made the amazing package `stars` (a little more about this [later](#homework))!!

Let's just see what we get by converting the `RasterLayer` object to a `stars` object and then apply `write_stars()`, hm?
...
Yay!\
It's interesting to see how the info at this level disregards CRS and coordinates.

When we query the metadata of one sublayer, it is seen that CRS and coordinates are layer-specific:
