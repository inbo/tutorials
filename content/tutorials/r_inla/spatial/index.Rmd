...
```{r spde1}
spde1 <- inla.spde2.pcmatern(mesh1, prior.range = c(100e3, 0.05), 
                             prior.sigma = c(0.9, 0.1))
spde2 <- inla.spde2.pcmatern(mesh2, prior.range = c(100e3, 0.05), 
                             prior.sigma = c(0.9, 0.1))
```
...
```{r model-fit-inlabru}
bru_spde1 <- bru(Rain ~ Xc + Yc + location(map = st_coordinates, model = spde1), 
