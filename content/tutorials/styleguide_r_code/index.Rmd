...
- to make data available to package users, store them in the `data` folder by using `usethis::use_data(xyz)`
- to make data available for loading and parsing examples, store them in the folder `inst/extdata`. Access this data with `system.file("xyz", package = "abc")`
...
- add scripts for generating these data in the folder `data-raw` and create this folder by using `usethis::use_data_raw()` (to ignore them during build)
