...
Hold that thought.
As will become clear in the end, the **assumption of Gaussian spatial interdependence** is what will allow the Matérn regression to work ([@sec-matern]).
...
De-trending often improves variogram model regression ([@sec-nodetrend]), but it also removes/diminishes the effects of spatially correlated co-variates.
...
## Distance-Difference Diagrams {#sec-distdiffplots}
...
#| eval: true
#| label: fig-difference-distance
#| fig-cap: "The difference-distance plot, visualized here as box-and-whiskers. Sorry for the ugly x-axis tick labels."
...
#| fig-cap: "The left panel shows the averaged difference-distance plot, whereas the right panel is the semivariance, plotted against the distance."
...
You might call the right one ([@fig-empirical-variogram], right) the "empirical variogram", if you like.
...
- Parameters can be measured, in this case intercept ($`{r} round(optimizer_results$par[1], 2)`$) and slope ($`{r} round(optimizer_results$par[2], 2)`$).
...
#| fig-cap: "Residual distribution of the Matérn model for semivariance."
plot_residuals_histogram(x, y, predictor_function,
  bins = 32, fill = "lightgray", color = "black")
```


The regression results are `{r} paste(round(optimizer_results$par, 2), collapse = ", ")`.
...
#| fig-cap: "Variogram of all the data, with a Matérn fit."
...
The $2\sigma$-range of the full data set with two different categories mixed is $`{r} round(2*vg_all$par[2], 1)`$.
If we only include one of the categories, making the data points more similar, ranges are $`{r} round(2*vg_b0$par[2], 1)`$ and $`{r} round(2*vg_b1$par[2], 1)`$.
...
Consequently, we see the `sill-nugget` increase from $`{r} round(vg_b0$par[1]-vg_b0$par[3], 2)`$ to $`{r} round(vg_b1$par[1]-vg_b1$par[3], 2)`$.
...
## Disabled De-trending {#sec-nodetrend}
...
It can be easily implemented, you can get an idea in the section on "smoothing" ([@sec-smoothing]) above.
