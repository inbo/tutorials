...
## Cross-Difference Convenience {#sec-crossdifference}
...
Note: on real data, you will probably not want to wrap.
...
#| fig-cap: "The left is the averaged difference-distance plot, whereas the right plot is the semivariance, plotted against the distance."
...
Vari-o-gram ([@fig-empirical-variogram], right).
...
```{r plot-matern-variogram}
#| label: fig-matern-variogram
#| fig-cap: "Residual distribution of the Matérn model for semivariance."
...
The range of the full data set with two different categories mixed is `{r} range`.
If we only include one of the categories, making the data points more similar, range is `{r} range_b1`.
...
You might as well interpolate with the more general [RBF](https://en.wikipedia.org/wiki/Radial_basis_function) (e.g. [using `scipy.interpolate.
