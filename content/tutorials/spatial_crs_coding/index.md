...
In the below code chunks, this is demonstrated for several core
geospatial R packages. Other geospatial R packages should normally
...
First, some practical notes:
  ...
  - In order to remain compatible with GDAL 2 and PROJ 4, some packages
    ...
    not enter a proj4string. Not that the derived proj4string will not
    ...
    about dropped keys in the proj4strings,\[3\] but in the meantime,
    for most geospatial R packages you can safely ignore this warning.
  - Windows users: to make sure that the `rgdal` and `sf` packages use
    GDAL 3 / PROJ ≥ 6 – which is now highly advised: please **update**
    all your packages in a pure R console (not RStudio) that uses your
    latest installed R version, by running `update.packages(ask = FALSE,
    checkBuilt = TRUE)`.
...
We now convert it to a plain dataframe with the XY coordinates as
columns, for the sake of the exercise. We want to add the CRS
ourselves\!
...
head(cities, 10) # top 10 features
```
...
#### Printing a WKT2 string with `sf`
...
There are a few extras to note:
  ...
  - printing the `crs` object provides both the EPSG code (more
    generally: the user’s CRS speficiation) and the WKT2 string:
...
#### Set the CRS of a Spatial\* object in `sf`
...
#### Get the CRS of a Spatial\* object in `sf`
...
The `sp` package heavily relies on the `rgdal`R package (to communicate
with GDAL and PROJ), so let’s check whether `rgdal` uses (for Windows:
comes with) the minimal PROJ/GDAL versions that we want.
...
``` r
crs_wgs84 <- CRS(SRS_string = "EPSG:4326") # WGS84
```
...
#### Printing a WKT2 string with `sp`
...
Also note that, when just printing a `CRS` object of `sp`, you would
still just get a proj4string printed (with parts dropped if on recent
GDAL/PROJ). We won’t print it here\! However do know that the WKT2
string is also *contained* in the `CRS` object, just call `wkt()` to see
it.
...
We prepare a few copies from the data:
...
take a multitude of values; these are all equivalent:
...
when creating the RasterLayer. It can take one of the above values.
...
3.  The packages give a warning especially to make developers of
    *other*, dependent packages *aware* that they should also make sure
    their functions do not *require* proj4strings or use harcoded
