...
We chose to make the spatial join in the `EPSG:31370` CRS. However, we can equally do it on a spherical model of the Earth, when using the geographical coordinates.
...
So the same information appears as when manually selecting and then printing, except for the number of 'candidate operations found':
...
It's obtained by dropping the `--spatial-test intersects` option (or replacing `intersects` by `contains`, which is default), and so it matches the pipeline where `accuracy` equals `NA` in our `pipelines` dataframe:
