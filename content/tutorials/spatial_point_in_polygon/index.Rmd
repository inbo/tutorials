...
Note the warning: it is because the spatialite database from the EEA website (with the 10 km x 10 km grid) still uses the older PROJ.4 string to represent the coordinate reference system.
Because the `rgdal` package is still backwards compatible, we should not worry about this: `rgdal` does the translation for the newer GDAL 3 and PROJ >= 6, which are not compatible with PROJ.4 strings.
Do know that, instead of _PROJ.4_ strings, the _WKT2_ string is now used in R to better represent coordinate reference systems.
Just compare these:
